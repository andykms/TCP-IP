...
function setupIpcHandlers() {
  ipcMain.handle("tcp:connect", async (event: IpcMainInvokeEvent, connectionId: string, host: string, port: number)=>{
  ...
  ipcMain.handle("tcp:send", async (event: IpcMainInvokeEvent, connectionId: string, data: string) => {
  ...
  ipcMain.handle("tcp:disconnect", async (event: IpcMainInvokeEvent, connectionId: string) => {
    const connection = connections.get(connectionId);
  ...
  ipcMain.handle("tcp:sendFile", async (event: IpcMainInvokeEvent, connectionId: string, filePath: string) => {
    const connection = connections.get(connectionId);
